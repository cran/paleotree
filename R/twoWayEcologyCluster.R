...
twoWayEcologyCluster<-function(
	...
	#				start=1,stop=5)
	#			,"...")
	rownames(propAbund) <- strtrim(rownames(propAbund), width = 5)
	colnames(propAbund) <- strtrim(colnames(propAbund), width = 5)
	#
	####################################
	## PLOTTING IT
	##
